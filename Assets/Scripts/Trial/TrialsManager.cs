...
public class TrialsManager : MonoBehaviour
{
    ...
    /// <summary>
    /// Populate list of all the object tagged as Manager
    /// Assign them to the needed variables
    /// </summary>
    public void getManagersInScene()
    {
        Managers = GameObject.FindGameObjectsWithTag("Manager");
        
        for (int i = 0; i < Managers.Length; i++ )
        {
            if (Managers[i].name.Contains("Event"))
            {
                eventManager = Managers[i].GetComponent(typeof(EventManager)) as EventManager;
    ...
    protected Trial currentTrial { get { return allTrialsData.Count == 0 ? null : allTrialsData.Last(); } }
    ...
    public void AddBounceToCurrentTrial()
    {
        ...
        //todo : have that work with difficulty definition 
        // now is a workabout to see if works
        switch (GlobalPreferences.Instance.startingDifficulty)
        {
            case DifficultyChoice.BASE:
                nbBounceToTrigger = GlobalPreferences.Instance.bouncePerDiffLevelToTriggerEvents[0];
                break;
            case DifficultyChoice.MODERATE:
                nbBounceToTrigger = GlobalPreferences.Instance.bouncePerDiffLevelToTriggerEvents[1];
                break;
            case DifficultyChoice.MAXIMAL:
                nbBounceToTrigger = GlobalPreferences.Instance.bouncePerDiffLevelToTriggerEvents[2];
                break;
        }
        
        if (currentTrial.nbBounces % nbBounceToTrigger == 0)
        {
    ...
    public bool AreDistractionCriteriaMet()
    {
        return difficultyManager.AreDistractionCriteriaMet(currentTrial);
    }
